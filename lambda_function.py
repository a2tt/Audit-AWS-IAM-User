...
def filter_exception(username, source, ip, event_name):
    ...
    if ip == "AWS Internal":
        return True
    if event_name in ["GetBucketLocation"]:
        return True
...
def lambda_handler(event: dict, context: dict = None):
    ...
    for trail_log in trail_logs:
        try:
            ...
            event_time = (dateutil.parser.parse(trail_data["eventTime"]) + datetime.timedelta(hours=9)).strftime(
                "%Y.%m.%d %H:%M:%S"
            )
            ...
            if filter_exception(username, event_source, ip, event_name) is True:
                continue
